...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                ...
                version="2.0">
   <xsl:import href="file:/C:/c-1/c1-data-generation/c1-data-generation.xsl"/>
   ...
   <xsl:variable name="x:stylesheet-uri"
                 as="xs:string"
                 select="'file:/C:/c-1/c1-data-generation/c1-data-generation.xsl'"/>
   ...
   <xsl:template name="x:d5e2">
      <xsl:message>when processing a para element</xsl:message>
      <x:scenario>
         <x:label>when processing a para element</x:label>
         <x:context>
            <documents/>
         </x:context>
   ...
   <xsl:template name="x:d5e5">
      ...
      <xsl:message>It should produce an xpf:map element</xsl:message>
      ...
      <x:test successful="{$impl:successful}">
         <x:label>It should produce an xpf:map element</x:label>
         <xsl:call-template name="test:report-value">
